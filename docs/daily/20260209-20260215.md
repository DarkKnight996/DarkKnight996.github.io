...
- **[arXiv260212] Chamfer-Linkage for Hierarchical Agglomerative Clustering**
  - **tags:** [ai], [clustering algorithms], [hierarchical agglomerative clustering, chamfer distance, linkage function]
  ...
  - **Simple LLM Summary:** This paper proposes Chamfer-linkage, a novel linkage function for Hierarchical Agglomerative Clustering (HAC) that uses the Chamfer distance to measure inter-cluster similarity. Theoretically, it maintains O(n²) time complexity, and experimentally, it consistently produces higher-quality clusterings than classical linkages like average-linkage and Ward's method across diverse datasets.

- **[arXiv260212] Implementability of Global Distributed Protocols modulo Network Architectures**
  - **tags:** [sys], [distributed systems verification], [global protocols, implementability, network architectures, coherence conditions, asynchronous communication, symbolic algorithms]
  - **authors:** Elaine Li, Thomas Wies
  - **institution:** New York University
  - **link:** https://arxiv.org/pdf/2602.10320
  - **Simple LLM Summary:** This paper introduces a network-parametric framework for verifying the implementability of global distributed protocols. It proposes a set of Coherence Conditions that, under minimal operational axioms, precisely characterize implementability across various asynchronous network architectures like FIFO and mailbox. The authors implement this characterization in a tool called Sprout(A), demonstrating its generality and performance.
...
- **[arXiv260212] Execution-Centric Characterization of FP8 Matrix Cores, Asynchronous Execution, and Structured Sparsity on AMD MI300A**
  ...
  - **Simple LLM Summary:** This paper uses targeted microbenchmarks to characterize the execution behavior of advanced accelerator features (FP8 matrix cores, asynchronous compute engines, and structured sparsity) on the AMD MI300A APU. It quantifies performance trade-offs related to occupancy, concurrency, and sparsity, providing practical guidance for scheduling and optimization on unified nodes.

- **[arXiv260212] Computing Least Fixed Points with Overwrite Semantics in Parallel and Distributed Systems**
  - **tags:** [sys], [parallel and distributed algorithms], [least fixed points, overwrite semantics, monotone inflationary functions, bounded staleness, i-locality, Knaster-Tarski theorem]
  ...
  - **Simple LLM Summary:** This paper presents methods for computing least fixed points of monotone inflationary functions in parallel and distributed systems using coordinate-wise overwrite semantics instead of traditional join-based merges. It proves convergence theorems under progressively relaxed synchronization conditions, including interleaving, update-only-on-change, and distributed execution with bounded staleness and locality. The results provide the first exact convergence guarantees for overwrite-based parallel updates without requiring join operations or contraction assumptions.
...
- **[arXiv260212] Fine-Tuning GPT-5 for GPU Kernel Generation**
  ...
  - **Simple LLM Summary:** This paper presents a reinforcement learning (RL) fine-tuning approach for GPT-5 to generate efficient GPU kernels, addressing the limitations of supervised fine-tuning due to scarce training data. The method uses a specialized environment and tools to improve kernel correctness and performance significantly. The results demonstrate that targeted RL post-training can effectively unlock LLM capabilities for specialized accelerator programming, outperforming existing compilers.
...
- **[arXiv260212] BOute: Cost-Efficient LLM Serving with Heterogeneous LLMs and GPUs via Multi-Objective Bayesian Optimization**
  ...
  - **Simple LLM Summary:** This paper introduces BOute, a system that uses multi-objective Bayesian optimization to co-optimize query routing strategies and model deployment across heterogeneous LLMs and GPUs for cost-efficient serving. It demonstrates that BOute significantly improves cost-efficiency, outperforming state-of-the-art systems by up to 157% under the same budget or reducing costs by 38% on average while meeting performance targets.

- **[arXiv260212] KORAL: Knowledge Graph Guided LLM Reasoning for SSD Operational Analysis**
  - **tags:** [mlsys], [llm inference], [knowledge graph, large language models, ontology, telemetry analysis, SSD diagnosis]
  ...
  - **Simple LLM Summary:** The paper presents KORAL, a framework that integrates Large Language Models with structured Knowledge Graphs to analyze Solid State Drive operations. It uses a Data KG from telemetry and a Literature KG from documents to guide the LLM in providing evidence-based, explainable analysis. The evaluation shows the system delivers expert-level diagnosis and actionable recommendations for SSD performance and reliability.

- **[arXiv260212] Min-Sum Uniform Coverage Problem by Autonomous Mobile Robots**
  - **tags:** [sys], [distributed robotics], [Look-Compute-Move model, non-rigid motion, fair asynchronous scheduler, oblivious robots, deterministic distributed algorithm]
  ...
  - **Simple LLM Summary:** The paper presents deterministic distributed algorithms for the min-sum uniform coverage problem for autonomous, oblivious robots on a line segment and a circle. For the line segment, it provides an algorithm achieving minimum total movement cost, and for the circle, it characterizes unsolvable initial configurations and provides a minimum-cost algorithm for all other cases. These results characterize the deterministic solvability and achieve optimal movement cost for the problem.
...
- **[arXiv260212] Authenticated Workflows: A Systems Approach to Protecting Agentic AI**
  ...
  - **Simple LLM Summary:** The paper introduces "authenticated workflows," a systems approach that provides deterministic security for agentic AI by cryptographically protecting boundaries (prompts, tools, data, context) and enforcing policies via the MAPL language. It concludes that this method eliminates entire classes of attacks by design and integrates with major AI frameworks, achieving 100% recall with zero false positives in empirical tests.
...
- **[arXiv260213] Real Life Is Uncertain. Consensus Should Be Too!**
  - **tags:** [sys], [distributed consensus], [probabilistic consensus, failure curves, quorum intersection, f-threshold model]
  ...
  - **Simple LLM Summary:** This paper proposes moving from a traditional deterministic f-threshold failure model to a probabilistic failure model for consensus protocols in distributed systems. It argues that probabilistic models better capture real-world fault complexities and can enable more reliable, efficient, and cost-effective systems by leveraging individual machine failure curves and avoiding bottlenecks like majority quorum intersection.

- **[arXiv260213] Differentially Private Perturbed Push-Sum Protocol and Its Application in Non-Convex Optimization**
  - **tags:** [mlsys], [others], [differential privacy, decentralized optimization, perturbed push-sum protocol, sensitivity estimation, partial communication]
  - **authors:** Yiming Zhou, Kaiping Xue, Enhong Chen
  - **institution:** University of Science and Technology of China
  - **link:** https://arxiv.org/pdf/2602.11544
  - **Simple LLM Summary:** The paper proposes DPPS, a differentially private protocol for decentralized communication, and its application PartPSP for non-convex optimization. The core method involves a novel sensitivity estimation mechanism and a partial communication strategy to reduce noise injection. The authors theoretically prove convergence and show experimentally that their approach outperforms existing privacy-preserving decentralized optimization algorithms.
...
- **[arXiv260213] Designing Scalable Rate Limiting Systems: Algorithms, Architecture, and Distributed Solutions**
  ...
  - **Simple LLM Summary:** The paper proposes a distributed rate limiting system using Redis with Sorted Sets and the Rolling Window algorithm, enhanced by atomic Lua scripting for concurrency control. It quantifies the trade-off between accuracy and memory cost compared to Token Bucket and Fixed Window algorithms. The architecture leverages Redis Cluster for scalability and adopts an AP (Availability and Partition Tolerance) compromise from the CAP theorem as a practical engineering choice.

- **[arXiv260213] MUSE: Multi-Tenant Model Serving With Seamless Model Updates**
  - **tags:** [mlsys], [others], [multi-tenant model serving, score calibration, decision thresholds, intent-based routing, two-level score transformation, reference distribution]
  ...
  - **Simple LLM Summary:** MUSE is a model serving framework that decouples model scores from client decision boundaries using a two-level score transformation to map outputs to a stable reference distribution, enabling seamless model updates. It employs dynamic intent-based routing for efficient multi-tenant infrastructure reuse. This approach reduces model update lead time from weeks to minutes, enhancing resilience against adversarial shifts and saving significant operational costs.

- **[arXiv260213] Future Mining: Learning for Safety and Security**
  - **tags:** [mlsys], [others], [multimodal perception, secure federated learning, reinforcement learning, DTN communication, energy-aware sensing, machine unlearning]
  - **authors:** Md Sazedur Rahman, Mizanur Rahman Jewel, Sanjay Madria
  - **institution:** Missouri University of Science and Technology
  - **link:** https://arxiv.org/pdf/2602.11472
  - **Simple LLM Summary:** The paper proposes a Unified Smart Safety and Security Architecture for mining, integrating modules for multimodal perception, secure federated learning, and DTN-enabled communication. Its core method is a cohesive framework designed to address challenges like poor connectivity and cyber-physical threats in harsh mining environments. The main conclusion is that this architecture can build a resilient and trustworthy intelligent mining system to safeguard miners and ensure operational continuity under adversarial conditions.

- **[arXiv260213] LoRA-based Parameter-Efficient LLMs for Continuous Learning in Edge-based Malware Detection**
  - **tags:** [mlsys], [llm inference], [LoRA, continuous learning, edge computing, parameter-efficient fine-tuning, transformer models, federated learning, malware detection]
  ...
  - **Simple LLM Summary:** This paper proposes a continuous learning architecture for edge-based malware detection using lightweight transformer models (e.g., DistilBERT) fine-tuned locally with LoRA adapters. The LoRA modules are aggregated and shared across devices to enable knowledge transfer without exchanging raw data. The method achieves up to 20-25% accuracy gains on unseen attacks while adding minimal memory overhead, making it practical for resource-constrained edge hardware.

- **[arXiv260213] GORGO: Maximizing KV-Cache Reuse While Minimizing Network Latency in Cross-Region LLM Load Balancing**
  - **tags:** [mlsys], [llm inference], [KV-cache reuse, prefix caching, load balancing, network-aware routing, centralized proxy, Time-to-First-Token (TTFT)]
  ...
  - **Simple LLM Summary:** The paper introduces GORGO, a load balancing method for cross-region LLM inference that optimizes a total serving cost function considering KV-cache reuse, network latency, and compute availability. It demonstrates that GORGO reduces tail latency (P99 TTFT) and improves average TTFT by preventing inefficient cross-region forwarding, with its centralized proxy version being 2.5x faster in median TTFT.

- **[arXiv260213] LAER-MoE: Load-Adaptive Expert Re-layout for Efficient Mixture-of-Experts Training**
  - **tags:** [mlsys], [llm training], [Fully Sharded Expert Parallel (FSEP), load balancing, expert parallelism, All-to-All communication, dynamic routing]
  ...
  - **Simple LLM Summary:** This paper introduces LAER-MoE, a training framework that uses a novel Fully Sharded Expert Parallel (FSEP) paradigm to dynamically re-layout expert parameters and balance computational load during Mixture-of-Experts model training. It achieves up to 1.69x acceleration over state-of-the-art systems by minimizing communication overhead and optimizing token routing.

- **[arXiv260213] PAM: Processing Across Memory Hierarchy for Efficient KV-centric LLM Serving System**
  - **tags:** [mlsys], [llm inference], [processing-in-memory, hierarchical memory, KV cache, attention computation, workload scheduling]
  ...
  - **Simple LLM Summary:** The paper proposes PAM, a KV-centric LLM serving system that coordinates heterogeneous PIM-enabled memory devices in a hierarchy to address the bandwidth and capacity bottlenecks of KV cache operations. It introduces techniques like context-locality-aware KV distribution, the PAMattention algorithm, and online scheduling to balance workloads across memory devices. The system enhances the efficiency and scalability of LLM serving by simultaneously meeting high bandwidth and large capacity demands.

- **[arXiv260213] RL over Commodity Networks: Overcoming the Bandwidth Barrier with Lossless Sparse Deltas**
  - **tags:** [mlsys], [post-training], [sparse delta checkpoint, multi-stream transmission, throughput- and bandwidth-aware scheduling, lease-based fault tolerance, lossless compression]
  ...
  - **Simple LLM Summary:** The paper presents SparrowRL, a system for efficient RL post-training of LLMs over commodity networks by exploiting the sparsity of parameter updates. It transmits only the changed parameters as lossless sparse deltas, pipelining and overlapping transfer with computation to reduce overhead. This approach significantly improves throughput and cost-efficiency compared to full-weight broadcasts, narrowing the performance gap with expensive RDMA clusters.

- **[arXiv260213] An Auction-Based Mechanism for Optimal Task Allocation and Resource Aware Containerization**
  - **tags:** [sys], [distributed computing], [auction-based bidding, Docker swarm, containerization, task offloading, resource allocation]
  - **authors:** Ramakant kumar
  - **institution:** Unknown
  - **link:** https://arxiv.org/pdf/2602.11998
  - **Simple LLM Summary:** The paper proposes AUC-RAC, an auction-based mechanism that uses Docker swarm and containerization to optimally allocate and offload computation tasks among local servers in IoT environments. Experimental results show the approach improves task allocation and provides efficient computation services by enabling cooperation between servers.

- **[arXiv260213] Contention Resolution, With and Without a Global Clock**
  - **tags:** [sys], [distributed algorithms], [contention resolution, global clock, local clock, randomized protocols, latency analysis, complexity separation]
  ...
  - **Simple LLM Summary:** The paper studies the Contention Resolution problem in distributed systems, comparing models with and without a global clock. It introduces a new protocol with improved latency in the GlobalClock model and proves complexity gaps between different performance metrics (in-expectation vs. with-high-probability) in the LocalClock model, showing no single protocol can be optimal for both.

- **[arXiv260213] OServe: Accelerating LLM Serving via Spatial-Temporal Workload Orchestration**
  - **tags:** [mlsys], [llm inference], [workload-aware scheduling, workload-adaptive switching, heterogeneous model deployment, spatial-temporal heterogeneity, data parallelism, tensor parallelism, pipeline parallelism]
  - **authors:** Youhe Jiang, Fangcheng Fu, Taiyi Wang, Guoliang He, Eiko Yoneki
  - **institution:** University of Cambridge, Shanghai Jiao Tong University
  - **link:** https://arxiv.org/pdf/2602.12151
  - **Simple LLM Summary:** OServe is an LLM serving system that addresses spatial and temporal workload heterogeneity through a workload-aware scheduling algorithm and an adaptive switching method for dynamic model deployment. It optimizes performance by tailoring parallelism strategies to real-time workload characteristics and predicting changes. Experiments on real-world traces show OServe improves throughput by up to 2× compared to state-of-the-art systems.

- **[arXiv260213] PrefillShare: A Shared Prefill Module for KV Reuse in Multi-LLM Disaggregated Serving**
  - **tags:** [mlsys], [llm inference], [KV cache reuse, disaggregated serving, prefill sharing, fine-tuning decode module, routing mechanism]
  ...
  - **Simple LLM Summary:** The paper proposes PrefillShare, a method that factorizes an LLM into separate prefill and decode modules, freezes the prefill module, and fine-tunes only the decode module to allow multiple specialized models to share a single prefill computation and KV cache for the same prompt prefix. It introduces a routing mechanism for sharing across heterogeneous models in a disaggregated vLLM-based system. The approach matches full fine-tuning accuracy while significantly reducing latency and increasing throughput in multi-model agent workloads.

- **[arXiv260213] Legitimate Overrides in Decentralized Protocols**
  - **tags:** [sys], [decentralized systems, blockchain governance], [emergency mechanisms, scope × authority taxonomy, stochastic cost-minimization, exploit incidents analysis, heavy-tailed distribution]
  ...
  - **Simple LLM Summary:** The paper develops a Scope × Authority taxonomy to map the design space of emergency override mechanisms in decentralized protocols and formalizes the trade-offs as a stochastic cost-minimization problem. By analyzing 705 exploit incidents, it finds that containment time varies by authority type, losses follow a heavy-tailed distribution, and community sentiment affects the cost of intervention, leading to concrete design principles for emergency governance.
...
- [arXiv260213] On the Complexity of Offline Reinforcement Learning with $Q^$-Approximation and Partial Coverage [link](https://arxiv.org/pdf/2602.12107)
